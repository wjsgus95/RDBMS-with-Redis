    ...
    if((((char*)c->argv[2]->ptr)[0]) == '\r') {
        ...
        for(int i = 2; i < c->argc; i++) {
            for(int j = 0; j < tableObj->column_length; j++) {
                ...
                iter++;
                tableObj->table[tableObj->length][j] = calloc(1, strlen(c->argv[i]->ptr)-iter);
                memcpy(tableObj->table[tableObj->length][j], (((char*)c->argv[i]->ptr)+iter), strlen(c->argv[i]->ptr)-iter);
            }
        }
    }
